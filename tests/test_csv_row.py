...
def test_csv_row_empty(mapping):
    ...
    assert repr(csv_row) == "CsvRow(date='', type='', partner='', amount=0.0, currency='', category_id='', account='', notice='', confidence=None)"
...
def test_csv_row_initialization(setup_data, mapping):
    ...
    assert csv_row.category_id == ''


def test_csv_row_repr(setup_data, mapping):
    csv_row = CsvRow(setup_data, mapping)

    expected_repr = "CsvRow(date='1990-01-01', type='deposit', partner='Foo Bar', amount=1000.0, currency='EUR', category_id='', account='', notice='', confidence=None)"
